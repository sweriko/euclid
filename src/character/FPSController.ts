...
export class FPSController {
  ...
  constructor(world: RAPIER.World, startPosition: Vector3, options: Partial<FPSControllerOptions> = {}) {
  ...
  update(deltaS: number): void {
    ...
    if (this.inputForward > 0) {
      speed = this.isSprinting ? this.options.sprintSpeed : this.options.walkSpeed;
    ...
    if (this.inputRight !== 0) {
      const strafeSpeed = (this.inputForward > 0 && this.isSprinting) 
  ...
  teleport(newPosition: Vector3): void {
    ...
    this.rigidBody.setTranslation({ x: newPosition.x, y: newPosition.y, z: newPosition.z }, true);
    this.verticalVelocity = 0;
    this.velocity.set(0, 0, 0);
  }
  
  getCameraPosition(): Vector3 {
    const eyeOffset = this.options.playerHeight / 2 + this.options.playerRadius * 0.8;
    return new Vector3(
      this.position.x,
