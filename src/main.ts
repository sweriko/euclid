...
// Stencil materials for different render passes
let stencilMaskMaterial: MeshBasicNodeMaterial;

// Fullscreen background quad (so background respects stencil)
...
// Create the stencil mask material - writes to stencil buffer only, no color/depth
function createStencilMaskMaterial(): MeshBasicNodeMaterial {
...
// Configure material to only render where stencil equals ref (inside portal)
function setStencilInsidePortal(material: THREE.Material): void {
...
// Configure material to only render where stencil does NOT equal ref (outside portal)
...
function initFullscreenBackground(): void {
  fullscreenScene = new THREE.Scene();
  fullscreenCamera = new THREE.OrthographicCamera(-1, 1, 1, -1, 0, 1);

  fullscreenMaterial = new THREE.MeshBasicMaterial({ color: 0x000000 });
  fullscreenMaterial.depthTest = false;
  fullscreenMaterial.depthWrite = false;

  fullscreenQuad = new THREE.Mesh(
...
function createPortalStencilMask(width: number, height: number): THREE.Mesh {
  const geometry = new THREE.PlaneGeometry(width, height);
  geometry.translate(0, height / 2, 0);

  stencilMaskMaterial = createStencilMaskMaterial();

  const mesh = new THREE.Mesh(geometry, stencilMaskMaterial);
  mesh.renderOrder = -1; // Render first to set up stencil
  return mesh;
}
...
function update(currentTime: number): void {
  ...
  // === STENCIL-BASED PORTAL RENDERING ===
  // Reverse approach: render portal scene first, then mask with current scene

  const currentBackground = currentScene.background;
  // IMPORTANT: scene.background ignores our stencil settings (internal material),
  // so we must disable it during stencil/masked passes and draw our own quad.
  ...
  // Step 2: Render the PORTAL scene first (fills entire screen with "other world")
  ...
  // Step 4.5: Draw CURRENT world's background outside the portal (stencil != 1)
  if (currentBackground instanceof THREE.Color) {
    fullscreenMaterial.color.copy(currentBackground);
  } else {
    // Fallback (shouldn't happen in this demo)
    fullscreenMaterial.color.set(0x000000);
  }
  setStencilOutsidePortal(fullscreenMaterial);
  renderer.render(fullscreenScene, fullscreenCamera);

  // Step 5: Render the CURRENT world where stencil == 0 (not inside portal)
  // This overwrites everything EXCEPT the portal region
  portalStencilMask.visible = false;
  if (outerSphere) {
    outerSphere.visible = !isInInnerRoom && sphereHasOutsidePart;
  }
...
async function init(): Promise<void> {
  ...
  renderer.autoClear = false; // We'll manage clearing ourselves
  (renderer as unknown as { stencil: boolean }).stencil = true; // Explicitly enable stencil
